...
public class TokenSenderController implements ITokenSenderController {
    ...
    @Override
    public ResponseEntity<ResponseDtout> checkTokenValidity(String token) throws InvalidTokenException, AccountAlreadyActivatedException, ExpiredToken, UserNotFoundException {
        this.tokenSender.checkTokenValidity(token);
        return ResponseEntity.ok(new ResponseDtout(TOKEN_VALID));
    }

    @Override
    public ResponseEntity<ResponseDtout> sendTokenStudent(String email) throws InvalidUserInformationException, MessagingException, AccountAlreadyActivatedException, UserNotFoundException {

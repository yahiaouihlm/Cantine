...
RETURNS TRIGGER AS $$
    BEGIN
        IF NEW.label = 'ADMIN' AND NOT check_admin_integrity_role(NEW.user_id) THEN
            ...
            RAISE NOTICE 'WRONG ROLE FOR USER %s, THE USER AND HIS IMAGE IS REMOVED', NEW.user_id;
        ELSIF NEW.label = 'STUDENT' AND NOT check_student_integrity_role(NEW.user_id) THEN
            DELETE FROM role WHERE user_id = NEW.user_id;
            CALL remove_user_with_his_image_for_wrong_role(NEW.user_id);
            RAISE NOTICE 'WRONG ROLE FOR USER %s, THE USER AND HIS IMAGE IS REMOVED', NEW.user_id;
        ELSE

...
export class AdminService {
    ...
    private UPDATE_ADMIN_URL = this.BASIC_ENDPOINT + '/updateAdmin/info';
    ...
    updateAdmin(admin: FormData) {
        ...
        return this.httpClient.post<NormalResponse>(this.UPDATE_ADMIN_URL, admin, {headers: headers}).pipe(
            catchError((error) => this.handleUpdateAdminErrors(error))
        );
    }
    ...
    private handleUpdateAdminErrors(error: HttpErrorResponse) {
        ...
        if (error.status == HttpStatusCode.BadRequest) {
            errorMessage = "Certains champs sont invalides";
            this.dialog.openDialog(errorMessage);
            ...
            errorMessage = "utilisateur  introuvable ou  bien  Votre  fonction au  sien de  Ecole  aston    est introuvable";
            localStorage.clear();
            this.dialog.openDialog(errorMessage);
            this.router.navigate([IConstantsURL.HOME_URL]).then(window.location.reload);
        } else if (error.status == HttpStatusCode.InternalServerError) {
            errorMessage = "Une erreur serveur est survenue lors de la modification de l'administrateur";
            localStorage.clear();
            this.dialog.openDialog(errorMessage);
            this.router.navigate([IConstantsURL.HOME_URL]).then(window.location.reload);
        } else {
